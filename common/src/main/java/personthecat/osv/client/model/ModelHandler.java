...
public class ModelHandler {
    ...
    private static void runGenerator(final String blockStatPath, final VariantDescriptor variant) {
        final ModelSettings cfg = variant.getForeground().getModel();
        final ModelGenerator gen = cfg.getType().getGenerator();

        try {
            gen.generateModels(variant, blockStatPath, ModelHandler::writeModel);
